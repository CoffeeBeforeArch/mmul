...
void parallel_transpose_vdppd_gemm(const double *A, const double *b, double *C,
...
static void serial_transpose_gemm_bench(benchmark::State &s) {
  ...
  // Main benchmark loop
  for (auto _ : s) {
    serial_transpose_vdppd_gemm(A, B, C, N);
  }
  ...
  for (auto _ : s) {
    ...
    for (std::size_t i = 0; i < num_threads - 1; i++) {
      ...
      threads.emplace_back([&] {
        parallel_transpose_vdppd_gemm(A, B, C, N, start_row, end_row);
      });
    }
    parallel_transpose_vdppd_gemm(A, B, C, N, end_row, end_row + n_rows);
